...
MAX_SAFE_BOOST = 8.0
VERSION = "v1.3.0 (Pro Audio: FDW, Hilbert & Psychoacoustic)"
...
def load_config():
    default_conf = {
        ...
        'smoothing_type': 'Psychoacoustic', # NEW default
        'fdw_cycles': 15, # NEW default
...
# --- NEW PRO DSP FUNCTIONS (v1.3.0) ---

def calculate_minimum_phase(mags_lin_fft):
    """
    Calculates Minimum Phase from Magnitude using the Cepstral method (Hilbert Transform relation).
    1. Log Magnitude
    2. Real Cepstrum (IFFT)
    3. Apply Window (Causality)
    4. FFT back to Frequency Domain -> Complex Min Phase
    """
    ...
    return np.angle(min_phase_complex) # Radians

def psychoacoustic_smoothing(freqs, mags, oct_bw=1/3.0):
    """
    Simulates psychoacoustic smoothing by:
    1. Smoothing heavily (e.g. 1/3 oct) to find the 'average energy'.
    2. Smoothing lightly (e.g. 1/12 oct) to keep peaks.
    3. Taking the MAX of both. This fills dips but preserves peaks.
    """
    # 1. Heavy smoothing (fills dips)
    mags_heavy, _ = apply_smoothing_std(freqs, mags, np.zeros_like(mags), oct_bw)
    
    # 2. Light smoothing (preserves peaks)
...
def apply_fdw_smoothing(freqs, phases, cycles):
    """
    Applies Frequency Dependent Windowing (FDW) logic to PHASE data.
    Instead of complex time-domain windowing, we use variable smoothing on the phase response.
    Smoothing width (octaves) is roughly proportional to 1/cycles.
    Low cycles (e.g. 6) = Heavy smoothing = Rejects room reflections (Direct sound).
    High cycles (e.g. 30) = Light smoothing = Includes room reflections.
    """
    # Approximation: Smoothing bandwidth in octaves ~ 1 / (cycles * constant)
    # A standard FDW of 15 cycles is roughly similar to 1/3 octave smoothing on phase.
    # We will use a sliding window average for phase.
    
    # Unwrap first
    phase_u = np.unwrap(np.deg2rad(phases))
    
    # Simple smoothing approach for phase based on FDW cycles
    # Effective smoothing width:
    oct_width = 2.0 / max(cycles, 1.0) 
    
    # Reuse standard smoothing function for phase
    # Create dummy mags
...
def apply_smoothing_std(freqs, mags, phases, octave_fraction=1.0):
    """Standard logarithmic smoothing."""
...
def generate_filter(freqs, raw_mags, raw_phases, crossovers, 
    ...
    # 1. APPLY SMOOTHING (Standard or Psychoacoustic)
    ...
    # Interpolate Magnitude to Linear Axis
    meas_mags = interpolate_response(freqs, smoothed_mags, freq_axis)
    
    # 2. PHASE CALCULATION (Minimum Phase & Excess Phase)
    # Calculate Minimum Phase from the Smoothed Magnitude
    meas_min_phase_rad = calculate_minimum_phase(10**(meas_mags/20.0))
    
    # Interpolate Measured Phase
    ...
    # Calculate Excess Phase (Total - Minimum)
    excess_phase_rad = meas_phase_rad_unwrapped - meas_min_phase_rad
    
    # 3. APPLY FDW (Frequency Dependent Windowing) to Excess Phase
    # We smooth the Excess Phase curve. 
    # Low cycles = Heavy smoothing (removes room hash). High cycles = Light smoothing.
    # Convert Excess Phase to degrees for the helper, then back
    ...
    for i, f in enumerate(freq_axis):
        if f > 0:
            ...
            # Phase Correction (Theoretical XO + Excess Phase FDW)
            fine_correction = 0.0
            if phase_c_min <= f <= phase_c_max:
                # We try to invert the FDW Excess Phase, limited by fine_phase_limit
...
def main():
    ...
    inputs_list = [
        ...
        select("Sample Rate", options=[44100, 48000, 96000], value=defaults['fs'], name='fs', type=NUMBER),
        select("Taps", options=[2048, 4096, 8192, 16384, 32768, 65536, 131072], value=defaults['taps'], name='taps', type=NUMBER),
        ...
        input("FDW Cycles", value=defaults.get('fdw_cycles', 15), type=FLOAT, name='fdw_cycles', help_text="Frequency Dependent Window cycles. 15 is standard. Lower = less room correction in phase."),
        ...
        input("Correction Min Freq (Hz)", value=defaults['hc_min'], type=FLOAT, name='hc_min'),
        input("Correction Max Freq (Hz)", value=defaults['hc_max'], type=FLOAT, name='hc_max'),
    ...
    try:
        ...
        update_status("Calculating filters (FDW & Minimum Phase)...")
