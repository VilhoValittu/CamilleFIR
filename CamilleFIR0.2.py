...
def ask_house_curve_file():
    ...
    if not candidates:
        print("\nNo suitable House Curve (.txt) files found in the folder.")
        return None
    ...
    while True:
        try:
            ...
            if 1 <= sel <= len(candidates): return candidates[sel-1]
            print("Invalid number.")
        except ValueError: print("Please enter a number.")
...
def generate_filter(freqs, smoothed_mags, smoothed_phases, crossovers, 
    ...
    # Calculate Group Delay (-dPhi/dw) from the final complex response
    # Unwrap phase first
    final_phase_unwrapped = np.unwrap(np.angle(final_complex))
    # Differentiate
    dw = (2 * np.pi * fs / 2.0) / len(freq_axis) # rad/sample approx
    gd_samples = -np.gradient(final_phase_unwrapped) * (len(freq_axis) / np.pi) 
    
    # Convert to ms
    gd_ms = (gd_samples / fs) * 1000.0
    
    # Filter for valid range (20Hz - 20kHz) to avoid DC/Nyquist noise
...
try:
    ...
    # Dictionary to store settings for summary
    settings = {}
    ...
    # 3. Sample Rate (Now needed before filename generation)
    print("\nSelect Sample Rate:")
    ...
    # 12. Generation
    print(f"\nGenerating filters...")
    # Modified call to unpack GD stats
